...
struct WebViewPage: Identifiable, Equatable {
    enum Source: Equatable {
        case remote(URL)
        case inlineHTML(String, baseURL: URL?)
    }
    ...
    static let defaults: [WebViewPage] = [
        .init(
            id: "demo-page",
            title: "Demo Page (inline HTML)",
            addressText: "inline://test-page/index.html",
            source: .inlineHTML(Self.demoPageHTML, baseURL: URL(string: "https://bitdrift.test/test-page/"))
        ),
        .init(
            id: "bitdrift-home",
            title: "bitdrift.io",
            addressText: "https://bitdrift.io/",
            source: .remote(URL(string: "https://bitdrift.io/")!)
        ),
        .init(
            id: "bitdrift-404",
            title: "bitdrift.io/hello (404)",
            addressText: "https://bitdrift.io/hello",
            source: .remote(URL(string: "https://bitdrift.io/hello")!)
        ),
        .init(
            id: "bitdrift-timeout",
            title: "bitdrift.ai (timeout)",
            addressText: "https://bitdrift.ai/",
            source: .remote(URL(string: "https://bitdrift.ai/")!)
        ),
        .init(
            id: "wikipedia",
            title: "Wikipedia",
            addressText: "https://www.wikipedia.org/",
            source: .remote(URL(string: "https://www.wikipedia.org/")!)
        )
    ]
}

// TODO: ademarco - add xcassets in the demo app and move this to an `.html` file  as android does.
private extension WebViewPage {
    static let demoPageHTML = #"""
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-900 min-h-screen p-4">
    ...
    <script type="module">
        ...
        const actions = {
            ...
            fetch: async (url, label) => {
                ...
                try {
                    ...
                    state.networkStatus = `${r.ok ? '✅' : '⚠️'} ${label}: ${r.status}`;
                } catch (e) { state.networkStatus = `❌ ${label}: ${e.message}`; }
                rerender();
            },
            xhr: () => {
                ...
                xhr.onload = () => { state.networkStatus = `✅ XHR: ${xhr.status}`; rerender(); };
                xhr.onerror = () => { state.networkStatus = '❌ XHR failed'; rerender(); };
                xhr.send();
            },
        ...
        const isSdkConfigured = () => window.bitdrift?.config;
        ...
        const App = () => html`
            ...
            ${!isSdkConfigured() ? html`
                <div class="bg-yellow-900 border border-yellow-600 rounded-lg p-4 shadow-md">
                    <h2 class="text-lg font-semibold text-yellow-200 mb-2">⚠️ SDK Not Configured</h2>
                    <p class="text-sm text-yellow-300">The Bitdrift SDK WebView integration is not configured. <code class="bg-yellow-800 px-1 rounded">window.bitdrift.config</code> was not found.</p>
                    <p class="text-sm text-yellow-400 mt-2">Please ensure the WebView is set up with the Bitdrift SDK's WebView integration to capture events.</p>
                </div>
            ` : ''}

            ${card('🎯', 'Web Vitals', 'LCP, CLS, INP, FCP, TTFB are automatically captured on page load.', html`
                <div class="space-y-2">
            ...
            ${card('🌐', 'Network', 'Trigger fetch/XHR requests to test network interception.', html`
            ...
            ${card('🧭', 'Navigation', 'Trigger SPA-style navigation events.', html`
                <div class="grid grid-cols-2 gap-2">
                ...
                <div class="grid grid-cols-2 gap-2 mt-2">
                    ...
                    <a href="secondary.html" class="bg-cyan-500 hover:bg-cyan-600 text-white py-2 px-3 rounded text-sm text-center transition">Navigate →</a>
                </div>
                <p class="mt-3 text-sm text-gray-400">Current path: ${state.currentPath}</p>
            `)}

            ${card('⏱️', 'Long Tasks', 'Block the main thread for >50ms to trigger long task detection.', html`
                ${btn('orange', state.longTaskBtn, actions.longTask, 'w-full')}
            `)}

            ${card('💔', 'Resource Errors', 'Inject broken resources to trigger error capture.', html`
            ...
            ${card('📝', 'Console Capture', 'Trigger various console methods.', html`
            ...
            ${card('👆', 'User Interactions', 'Click various elements to test interaction tracking.', html`
                <div class="space-y-2">
                ...
                <div class="mt-4">
                    <p class="text-sm text-gray-400 mb-2">Redacted Content Test (should be masked in logs):</p>
                    <div class="bg-gray-700 rounded p-3 space-y-2">
                        <input data-redacted type="text" value="john.doe@example.com" placeholder="Email" class="w-full bg-gray-600 text-gray-200 text-sm px-3 py-2 rounded" />
                        <input data-redacted type="password" value="MySecretPass123" placeholder="Password" class="w-full bg-gray-600 text-gray-200 text-sm px-3 py-2 rounded" />
                        <input data-redacted type="tel" value="(555) 123-4567" placeholder="Phone" class="w-full bg-gray-600 text-gray-200 text-sm px-3 py-2 rounded" />
                    </div>
                </div>
                <div class="mt-4">
            ...
            ${card('💥', 'Errors & Promise Rejections', 'Trigger uncaught errors and unhandled rejections.', html`
                <div class="grid grid-cols-3 gap-2">
            ...
            ${card('💥', 'Memory Leak', 'Create memory leak when tapping multiple times.', html`
                ${btn('red', 'Create Memory Leak (~100MB)', actions.createMemoryLeak, 'w-full bg-red-600 hover:bg-red-700')}
                ${state.memoryLeakStatus ? html`<p class="mt-3 text-sm text-yellow-400">⚠️ ${state.memoryLeakStatus}</p>` : ''}
            ...
            ${card('📄', 'Page View', 'Page view is tracked automatically. Use navigation buttons to trigger new page views.', html`
