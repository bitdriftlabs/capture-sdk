...
/**
 * Generates a [TraceContext] which will be added to network headers. This is gated by [io.bitdrift.capture.common.RuntimeStringConfig.TRACE_PROPAGATION_MODE]
 */
internal class TraceContextFactory {
    private val secureRandom = SecureRandom()

    // Pre-computed lookup table mapping each byte value (0–255) to its
    // two-character lowercase hex representation.
    private val byteToHexTable =
        Array(256) { index ->
    ...
    private fun generateRandomHex(byteCount: Int): String {
        ...
        for (byte in bytes) {
            output.append(byteToHexTable[byte.toInt() and 0xFF])
        }
    ...
    internal companion object {
        ...
        internal const val LOWER_HEX_DIGITS = "0123456789abcdef"
    }
}
