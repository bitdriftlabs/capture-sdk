...
class PreInitInMemoryLoggerTest {
    ...
    @Test
    fun `span ended before the SDK finishes starting drops the end log`() {
        // Known limitation: a Span has no real logger to send its end log to until the SDK
        // finishes starting, so end() called before that point is a no-op for that log.
        ...
        val attributes = argumentCaptor<LogAttributesOverrides>()
        verify(logger, times(1)).logInternal(
            eq(LogType.SPAN),
            any(),
            any(),
            eq(ArrayFields.EMPTY),
