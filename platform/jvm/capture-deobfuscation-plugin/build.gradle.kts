...
gradlePlugin {
    plugins {
        create("CaptureDeobfuscationPlugin") {
            id = "io.bitdrift.capture-deobfuscation-plugin"
            implementationClass = "io.bitdrift.capture.CaptureDeobfuscationPlugin"
        }
    }
}
...
mavenPublishing {
    ...
    pom {
        name.set("CaptureDeobfuscationPlugin")
        description.set("Official Capture Deobfuscation Gradle plugin.")
        url.set("https://bitdrift.io")
        licenses {
            license {
                name.set("BITDRIFT SOFTWARE DEVELOPMENT KIT LICENSE AGREEMENT")
                url.set("https://dl.bitdrift.io/sdk/android-maven/io/bitdrift/capture-deobfuscation-plugin/${findProperty("VERSION_NAME")}/LICENSE.txt")
                distribution.set("repo")
            }
            license {
                name.set("NOTICE")
                url.set("https://dl.bitdrift.io/sdk/android-maven/io/bitdrift/capture-deobfuscation-plugin/${findProperty("VERSION_NAME")}/NOTICE.txt")
                distribution.set("repo")
            }
        }
        developers {
