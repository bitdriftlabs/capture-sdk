package io.bitdrift.flappychippy.viewmodel
...
import io.bitdrift.flappychippy.util.DensityUtil
import io.bitdrift.flappychippy.util.LogUtil
import io.bitdrift.flappychippy.model.*
            ...
            withContext(Dispatchers.Default) {
                emit(when (action) {
                    ...
                    GameAction.HitGround -> run {
                        if (state.gameStatus !== GameStatus.Over) {
                            Logger.logDebug { "Chippy hit the ground " }
                        }
                    ...
                    GameAction.HitPipe -> run {
                        ...
                        Logger.logDebug { "Chippy hit a log "}
                    ...
                    GameAction.CrossedPipe -> run {
                        ...
                        Logger.logDebug(mapOf("best_score" to bestScore.toString(), "score" to state.score.toString())) { "Chippy crossed a log" }
                        state.copy(
                            pipeStateList = newPipeStateList,
                            score = state.score + 1,
                            bestScore = bestScore
