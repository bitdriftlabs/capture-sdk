...
pub extern "C" fn test_create_extremely_complex_nested_structure() -> *const Object {
    unsafe {
        // Create the most deeply nested structure - level 10
        ...
        level7_dict.insert("strings_showcase".to_string(), Value::Array(vec![
        ...
        // Level 6 - Array containing mixed nested structures
        ...
        // Level 5 - Dictionary with alternating structure types
        let mut level5_dict = HashMap::new();
        level5_dict.insert("0_key".to_string(), level6_array);
        level5_dict.insert("1_key".to_string(), Value::String("level5_value1".to_string()));
        level5_dict.insert("2_key".to_string(), Value::Array(vec![
        ...
        // Level 4 - Large array with various nested structures
        ...
        // Level 3 - Dictionary with comprehensive data types
        ...
        level3_dict.insert("single_item_array".to_string(), Value::Array(vec![
            Value::String("lonely_item".to_string())
        ]));
        level3_dict.insert("single_item_dict".to_string(), {
            let mut single_dict = HashMap::new();
            single_dict.insert("only_key".to_string(), Value::String("only_value".to_string()));
            Value::Object(single_dict)
        });
        level3_dict.insert("boolean_tests".to_string(), Value::Array(vec![
            Value::Bool(true), Value::Bool(false), Value::Bool(true), Value::Bool(false),
        ]));
        level3_dict.insert("null_tests".to_string(), Value::Array(vec![
            Value::Null, Value::String("not_null".to_string()), Value::Null,
        ]));
        
        // Level 2 - Array with alternating complex structures
        ...
        // Level 1 - Root dictionary with all complexity
        let mut root_dict = HashMap::new();
        root_dict.insert("complex_structure".to_string(), level2_array);
        root_dict.insert("metadata".to_string(), Value::Object({
            let mut meta = HashMap::new();
            meta.insert("version".to_string(), Value::String("1.0.0".to_string()));
            meta.insert("test_type".to_string(), Value::String("extreme_nesting".to_string()));
            meta.insert("nesting_levels".to_string(), Value::Signed(10));
            meta.insert("total_elements".to_string(), Value::Signed(500)); // approximate
            meta.insert("created_by".to_string(), Value::String("rust_ffi_test".to_string()));
            meta
        }));
        root_dict.insert("edge_cases".to_string(), Value::Object({
        ...
        root_dict.insert("stress_arrays".to_string(), Value::Array({
            ...
            for i in 0..50 {
                stress_arrays.push(Value::Array(vec![
                    ...
                    if i % 5 == 0 { Value::Null } else { Value::String(format!("non_null_{}", i)) },
                ]));
            }
            stress_arrays
        }));
        ...
        match value_to_objc(&root_value) {
            Ok(result) => *result,
            Err(_) => std::ptr::null(),
        }
    }
...
pub extern "C" fn test_complex_nested_round_trip() -> i32 {
    unsafe {
        ...
        match copy_from_objc(objc_structure) {
            Ok(_value) => {
