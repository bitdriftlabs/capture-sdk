...
#[no_mangle]
extern "C" fn capture_process_issue_reports(mut logger_id: LoggerId<'_>, report_processing_session_value: i32) {
  with_handle_unexpected(
    || -> anyhow::Result<()> {
      let session = match report_processing_session_value {
        ...
        _ => {
          log::error!("invalid session value: {report_processing_session_value}, defaulting to Current");
