...
/// - `NSArray` → `Value::Array`
/// - `NSDictionary` → `Value::Object` (HashMap)
/// - `NSNull` → `Value::Null`
///
/// # Algorithm
...
/// ```rust
/// // Convert an NSString
/// let ns_string = make_nsstring("hello");
/// let value = copy_from_objc(*ns_string)?;
/// assert_eq!(value, Value::String("hello".to_string()));
///
/// // Convert a nested NSDictionary with NSArray
/// let result = copy_from_objc(complex_dict_ptr)?;
/// // Results in nested Value::Object containing Value::Array elements
/// ```
      ...
      WorkItem::ProcessObject { ptr, result_id } => {
        if msg_send![ptr, isKindOfClass: class!(NSDictionary)] {
          ...
          if count > 0 {
            ...
            work_stack.push(WorkItem::FinalizeDictionary { map_id, result_id });
          ...
          if count > 0 {
            ...
            for i in (0 .. count).rev() {
              ...
              // Schedule the insertion after the value is processed
              work_stack.push(WorkItem::InsertArrayValue { value_id, array_id });
        ...
        match value {
          ...
          Value::Object(map) => {
            if map.is_empty() {
              ...
              // Schedule finalization
              work_stack.push(WorkItem::FinalizeDict { dict_id, result_id });
