...
/*
 * Reports an error to the bitdrift backend and log it to the console. Both reporting to remote and
 * logging to the console are throttled to protect against noisy errors.
 */
 ...
 * @param resource_utilization_target responsible for emitting resource utilization logs in response
 * to provided ticks.
 * @param session_replay_target responsible for emitting session replay logs in response to
 * callbacks.
 * @param events_listener_target responsible for listening to platform events and emitting logs in
 * response to them.
 ...
 * @param build_number the build number as a null terminated C string.
 * @param os_version the OS version as a null terminated C string.
 * @param os_brand the OS brand as a null terminated C string.
...
logger_id capture_create_logger(
    const char *_Nullable path, const char *api_key,
    id<SessionStrategyProvider> session_strategy_provider, id<MetadataProvider> metadata_provider,
    id<ResourceUtilizationTarget> resource_utilization_target,
    id<SessionReplayTarget> session_replay_target, id<EventsListenerTarget> events_listener_target,
    const char *app_id, const char *app_version, const char *build_number, const char *os_version,
    const char *os_brand, const char *model, _Nullable id<Network> network,
    _Nullable id<RemoteErrorReporting> error_reporter, bool start_in_sleep_mode);
 ...
 *        remote services.
 * @param matching_fields The list of matching fields that can be read when processing a given log
 * but are not a part of the log itself.
 * @Param blocking whether the method should return only after the log is processed.
 */
void capture_write_log(logger_id logger_id, int32_t log_level, uint32_t log_type,
                       const char *message, const NSArray<const Field *> *_Nullable fields,
                       const NSArray<const Field *> *_Nullable matching_fields, bool blocking,
 ...
 */
void capture_write_session_replay_screen_log(logger_id logger_id,
 ...
 * @param duration_s the duration of time the preparation of the session replay log took.
 */
void capture_write_session_replay_screenshot_log(logger_id logger_id,
                                                 const NSArray<const Field *> *fields,
                                                 double duration_s);
 ...
 */
void capture_write_resource_utilization_log(logger_id logger_id,
                                            const NSArray<const Field *> *fields,
                                            double duration_s);
 ...
 */
void capture_write_sdk_start_log(logger_id logger_id, const NSArray<const Field *> *fields,
                                 double duration_s);
 ...
 */
bool capture_should_write_app_update_log(logger_id logger_id, NSString *app_version,
 ...
 */
void capture_write_app_update_log(logger_id logger_id, NSString *app_version,
                                  NSString *build_number, uint64_t app_install_size_bytes,
                                  double duration_s);

/*
 * Writes an app launch TTI log. The method should be called only once per logger Id. Consecutive
 * calls have no effect.
 *
 * @param loggerId the ID of the logger to write to.
 * @param duration_s the duration of time between a user's intent to launch an app and the point in
 * time when the app became interactive. Calls with a negative duration are ignored.
 */
void capture_write_app_launch_tti_log(logger_id logger_id, double duration_s);
 ...
 */
void capture_write_screen_view_log(logger_id logger_id, NSString *screen_name);
...
/*
 * Returns the device ID. The ID is generated the first time it is accessed. Consecutive calls to
 * this method return the same value.
 *
 * @param logger_id the logger to use.
 */
...
/*
 * Removes a field with a given key. This operation does nothing if the field with the given key is
 * not registered with the logger.
 *
 ...
 */
bool capture_runtime_bool_variable_value(logger_id logger_id, const char *variable_name,
                                         bool default_value);
 ...
 */
uint32_t capture_runtime_uint32_variable_value(logger_id logger_id, const char *variable_name,
 ...
 * @param logger_id the logger to use for metadata
 * @param sdk_version bitdrift's React Native SDK version(e.g 8.1)
 */
void capture_persist_javascript_error_report(const char *error_name, const char *error_message,
                                             const char *stack_trace, bool is_fatal,
                                             const char *engine, const char *_Nullable debug_id,
                                             uint64_t timestamp_seconds, uint32_t timestamp_nanos,
                                             const char *destination_path, logger_id logger_id,
